...
about_box (void)
{
    ...
    struct tm *tm_parts = localtime (&now);
    int current_year = 2025;

    if (tm_parts != NULL)
    {
        int detected_year = tm_parts->tm_year + 1900;
    ...
    label_river = g_strdup_printf (_ ("Copyright (C) 2025-%d River Microelectronics"),
                                   current_year);
        ...
        quick_widget_t quick_widgets[] = {
            ...
            QUICK_NEXT_COLUMN,
                QUICK_START_GROUPBOX (N_ ("Other options")),
                    QUICK_CHECKBOX (N_ ("Use internal &editor (mcedit)"), &use_internal_edit, NULL),
