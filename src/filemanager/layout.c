...
#include "dirwatch.h"  // dirwatch_set_enabled()
...
/* Update panel file lists dynamically when directories change */
gboolean dynamic_file_list = FALSE;
/* Debounce interval (sec) for dynamic updates */
int dynamic_file_list_debounce_sec = 1;
...
#define B_DEB_PLUS           (B_USER + 4)
#define B_DEB_MINUS          (B_USER + 5)
...
static struct
{
    ...
    { N_ ("&Dynamic file list"), &dynamic_file_list, NULL },
};

static const char *output_lines_label = NULL;
static int output_lines_label_len;
static const char *debounce_label = NULL;
static int debounce_label_len;
static WLabel *debounce_value_widget = NULL;

static WButton *bleft_widget, *bright_widget;
static WButton *bdeb_plus_widget, *bdeb_minus_widget;
...
static int
bdebplus_cback (WButton *button, int action)
{
    (void) button;
    (void) action;
...
static int
bdebminus_cback (WButton *button, int action)
{
    (void) button;
    (void) action;
...
layout_bg_callback (Widget *w, Widget *sender, widget_msg_t msg, int parm, void *data)
{
    ...
    case MSG_DRAW:
        ...
        // Debounce label/value (dynamic updates)
        if (old_layout.debounce_ms != _debounce_sec)
        {
            char buf[8];
            old_layout.debounce_ms = _debounce_sec;
            g_snprintf (buf, sizeof (buf), "%02d", _debounce_sec);
            if (debounce_value_widget != NULL)
                label_set_text (debounce_value_widget, buf);
        }
        return MSG_HANDLED;
...
layout_callback (Widget *w, Widget *sender, widget_msg_t msg, int parm, void *data)
{
    ...
    case MSG_NOTIFY:
...

        {
            ...
            {
                dynamic_file_list = check_options[7].widget->state;
                if (bdeb_plus_widget != NULL)
                    widget_disable (WIDGET (bdeb_plus_widget), !dynamic_file_list);
                if (bdeb_minus_widget != NULL)
                    widget_disable (WIDGET (bdeb_minus_widget), !dynamic_file_list);
            }
            else
                ok = FALSE;
...
layout_dlg_create (void)
{
    ...
    const char *title4 = N_ ("Dynamic file list");
    ...
    debounce_label_len = str_term_width1 (debounce_label);
    l1 = max (l1, debounce_label_len + 12);
    // buttons
    ...
    layout_dlg = dlg_create (TRUE, 0, 0, 19, width, WPOS_CENTER, FALSE, dialog_colors,
...
#define XTRACT(i) (*check_options[i].variable != 0), check_options[i].text
    ...
    // "Dynamic file list" groupbox
    {
        widget_state_t disabled = dynamic_file_list ? 0 : WST_DISABLED;
        Widget *w;

        w = WIDGET (groupbox_new (11, 3, 3, l1, title4));
        w->state |= disabled;
        group_add_widget (g, w);

        // Debounce controls inside this group
        // Caption
        group_add_widget (g, label_new (12, 5, debounce_label));
        // Value label showing current ms
        {
            char buf[8];
            g_snprintf (buf, sizeof (buf), "%02d", _debounce_sec);
            debounce_value_widget = label_new (12, 5 + 3 + debounce_label_len, buf);
            group_add_widget (g, debounce_value_widget);
        }

        // +/- buttons
        w = WIDGET (button_new (12, debounce_label_len + 5, B_DEB_PLUS, NARROW_BUTTON, "&+",
                                bdebplus_cback));
        w->state |= disabled;
        group_add_widget (g, w);
        bdeb_plus_widget = BUTTON (w);

        w = WIDGET (button_new (12, debounce_label_len + 5 + 5, B_DEB_MINUS, NARROW_BUTTON, "&-",
                                bdebminus_cback));
        w->state |= disabled;
        group_add_widget (g, w);
        bdeb_minus_widget = BUTTON (w);
    }

    // "Other options" groupbox
    group_add_widget (g, groupbox_new (2, 4 + l1, 10, l1, title3));
...
#undef XTRACT
    ...
    group_add_widget (g, hline_new (15, -1, -1));
    // buttons
    group_add_widget (g, button_new (16, (width - b) / 2, B_ENTER, DEFPUSH_BUTTON, ok_button, 0));
    group_add_widget (
        g, button_new (16, (width - b) / 2 + b1 + 1, B_CANCEL, NORMAL_BUTTON, cancel_button, 0));

    // (Debounce controls moved into "Dynamic file list" groupbox above)
...
layout_save (void)
{
    ...
    old_layout.dynamic_file_list = dynamic_file_list;
    old_layout.output_lines = -1;
    old_layout.debounce_ms = -1;

    _output_lines = output_lines;
    _debounce_sec = dynamic_file_list_debounce_sec;
...
layout_restore (void)
{
    ...
    dynamic_file_list = old_layout.dynamic_file_list;
    output_lines = old_layout.output_lines;
    dynamic_file_list_debounce_sec = old_layout.debounce_ms;
...
layout_box (void)
{
    ...
    if (dlg_run (layout_dlg) == B_ENTER)
    {
        ...
        dynamic_file_list_debounce_sec = _debounce_sec;
        /* Apply dynamic watcher enable/disable if changed */
        dirwatch_set_enabled (dynamic_file_list);
    }
    else
        layout_restore ();
